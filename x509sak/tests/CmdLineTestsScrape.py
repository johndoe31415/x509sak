...
class CmdLineTestsScrape(BaseTest):
	...
	def test_scrape_dir_exists(self):
		with tempfile.TemporaryDirectory() as tempdir, WorkDir(tempdir), tempfile.NamedTemporaryFile(mode = "wb", prefix = "scrapeme_", suffix = ".bin") as f:
			...
			with open("scrape/foo", "wb"):
				pass
			SubprocessExecutor.run(self._x509sak + [ "scrape", "out.bin" ], success_retcodes = [ 1 ])
			self.assertTrue(os.path.isfile("scrape/foo"))
			SubprocessExecutor.run(self._x509sak + [ "scrape", "--force", "out.bin" ])
			self.assertTrue(os.path.isfile("scrape/foo"))
	...
	def test_scrape_random(self):
		with tempfile.TemporaryDirectory() as tempdir, WorkDir(tempdir):
			...
			SubprocessExecutor.run(self._x509sak + [ "scrape", "out.bin" ])
	...
	def test_scrape_der_crt(self):
		with tempfile.TemporaryDirectory() as tempdir, WorkDir(tempdir):
			...
			for prefix_len in [ 0, 100, 1000, 1024 * 1024 - 100, 1024 * 1024, 1024 * 1024 + 100 ]:
				prng = HashedPRNG(seed = b"foobar").write_bracketed("out.bin", prefix_len, crt.der_data, 1000)
				SubprocessExecutor.run(self._x509sak + [ "scrape", "out.bin" ])
				found = os.listdir("scrape/")
	...
	def test_scrape_pem_crt(self):
		with tempfile.TemporaryDirectory() as tempdir, WorkDir(tempdir), tempfile.NamedTemporaryFile(mode = "wb", prefix = "scrapeme_", suffix = ".bin") as f:
			...
			for prefix_len in [ 0, 100, 1000 ]:
				...
				SubprocessExecutor.run(self._x509sak + [ "scrape", "--no-der", f.name ])
				found = os.listdir("scrape/")
				self.assertEqual(len(found), 1)
				scraped_crt = X509Certificate.read_pemfile("scrape/scrape_%07x_crt.pem" % (prefix_len))[0]
				self.assertEqual(crt, scraped_crt)
	...
	def test_scrape_der_crt_twice(self):
		with tempfile.TemporaryDirectory() as tempdir, WorkDir(tempdir), tempfile.NamedTemporaryFile(mode = "wb", prefix = "scrapeme_", suffix = ".bin") as f:
			...
			SubprocessExecutor.run(self._x509sak + [ "scrape", "--no-pem", f.name ])
			...
			SubprocessExecutor.run(self._x509sak + [ "scrape", "--force", "--allow-non-unique-blobs", f.name ])
			found = os.listdir("scrape/")
	...
	def test_scrape_json(self):
		with tempfile.TemporaryDirectory() as tempdir, WorkDir(tempdir), tempfile.NamedTemporaryFile(mode = "wb", prefix = "scrapeme_", suffix = ".bin") as f:
			crt = self._load_crt("johannes-bauer.com.crt")
			self._prepare_file(f, [ 1000, crt.der_data, 100, crt.der_data, 500 ])
			SubprocessExecutor.run(self._x509sak + [ "scrape", "--write-json", "out.json", f.name ])
	...
	def test_extract_nested(self):
		with tempfile.TemporaryDirectory() as tempdir, WorkDir(tempdir), tempfile.NamedTemporaryFile(mode = "wb", prefix = "scrapeme_", suffix = ".bin") as f:
			...
			SubprocessExecutor.run(self._x509sak + [ "scrape", "--extract-nested", f.name ])
			found = os.listdir("scrape/")
			self.assertEqual(len(found), 2)
	...
	def test_failed_rsa_privkey(self):
		with tempfile.TemporaryDirectory() as tempdir, WorkDir(tempdir), tempfile.NamedTemporaryFile(mode = "wb", prefix = "scrapeme_", suffix = ".bin") as f:
			crt = self._load_crt("johannes-bauer.com.crt")
			self._prepare_file(f, [ 100, crt.der_data, 100 ])
			output = SubprocessExecutor.run(self._x509sak + [ "scrape", "--keep-original-der", f.name ])
			scraped_crt = X509Certificate.read_derfile("scrape/scrape_%07x_crt.der" % (100))
			self.assertEqual(len(os.listdir("scrape/")), 1)
			self.assertEqual(crt, scraped_crt)
	...
	def test_failed_rsa_plausibility(self):
		with tempfile.TemporaryDirectory() as tempdir, WorkDir(tempdir), tempfile.NamedTemporaryFile(mode = "wb", prefix = "scrapeme_", suffix = ".bin") as f:
			...
			SubprocessExecutor.run(self._x509sak + [ "scrape", f.name ])
			self.assertEqual(len(os.listdir("scrape/")), 0)
			shutil.rmtree("scrape")
			SubprocessExecutor.run(self._x509sak + [ "scrape", "--disable-der-sanity-checks", f.name ])
			self.assertEqual(len(os.listdir("scrape/")), 1)
	...
	def test_included_types(self):
		with tempfile.TemporaryDirectory() as tempdir, WorkDir(tempdir), tempfile.NamedTemporaryFile(mode = "wb", prefix = "scrapeme_", suffix = ".bin") as f:
			crt = self._load_crt("johannes-bauer.com.crt")
			self._prepare_file(f, [ 100, crt.der_data, 100 ])
			SubprocessExecutor.run(self._x509sak + [ "scrape", "--include-dertype", "crt", f.name ])
			self.assertEqual(len(os.listdir("scrape/")), 1)
			shutil.rmtree("scrape")

			SubprocessExecutor.run(self._x509sak + [ "scrape", "--include-dertype", "crt", "--exclude-dertype", "crt", f.name ])
			self.assertEqual(len(os.listdir("scrape/")), 0)
			shutil.rmtree("scrape")

			SubprocessExecutor.run(self._x509sak + [ "scrape", "--include-dertype", "pubkey", f.name ])
			self.assertEqual(len(os.listdir("scrape/")), 1)
			shutil.rmtree("scrape")

			SubprocessExecutor.run(self._x509sak + [ "scrape", "--include-dertype", "ec_key", f.name ])
			self.assertEqual(len(os.listdir("scrape/")), 0)
			shutil.rmtree("scrape")
