...
class CmdLineTestsDumpKey(BaseTest):
	def test_dump_rsa_privkey(self):
		(success, output) = SubprocessExecutor.run(self._x509sak + [ "dumpkey", "x509sak/tests/data/privkey_rsa_768.pem" ], return_stdout = True)
	...
	def test_dump_rsa_pubkey(self):
		(success, output) = SubprocessExecutor.run(self._x509sak + [ "dumpkey", "--public-key", "x509sak/tests/data/pubkey_rsa_768.pem" ], return_stdout = True)
	...
	def test_dump_ecc_privkey(self):
		(success, output) = SubprocessExecutor.run(self._x509sak + [ "dumpkey", "--key-type", "ecc", "x509sak/tests/data/privkey_ecc_secp256r1.pem" ], return_stdout = True)
	...
	def test_dump_ecc_pubkey(self):
		(success, output) = SubprocessExecutor.run(self._x509sak + [ "dumpkey", "--key-type", "ecc", "--public-key", "x509sak/tests/data/pubkey_ecc_secp256r1.pem" ], return_stdout = True)
