...
def genparser(parser):
	...
	parser.add_argument("-f", "--format", choices = [ "ansitext", "text", "json" ], default = "ansitext", help = "Determine the output format. Can be one of %(choices)s, defaults to %(default)s.")
	parser.add_argument("--json-input", action = "store_true", help = "Instead of reading in certificates and creating the respective output, read in a previously generated JSON file. Useful if you don't want to re-scan a certificate, but just change the display.")
