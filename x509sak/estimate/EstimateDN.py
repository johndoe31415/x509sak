...
class DistinguishedNameSecurityEstimator(BaseEstimator):
	...
	def _analyze_rdn_item(self, rdn_item):
		...
		if asn1type in self._VALID_ALPHABETS:
			...
			if len(illegal_chars) > 0:
				judgements += SecurityJudgement(JudgementCode.X509Cert_Body_FIXME_IllegalCharacter, "Distinguished name contains character(s) \"%s\" which are invalid for a %s at element \"%s\"." % ("".join(sorted(illegal_chars)), asn1type.__name__, OIDDB.RDNTypes.get(rdn_item.oid, str(rdn_item.oid))), compatibility = Compatibility.STANDARDS_DEVIATION)

		if isinstance(rdn_item.asn1, pyasn1.type.char.TeletexString):
			judgements += SecurityJudgement(JudgementCode.X509Cert_Body_FIXME_DeprecatedType, "Distinguished name contains deprecated TeletexString at element \"%s\"." % (OIDDB.RDNTypes.get(rdn_item.oid, str(rdn_item.oid))), compatibility = Compatibility.STANDARDS_DEVIATION)

		if not rdn_item.decodable:
			judgements += SecurityJudgement(JudgementCode.X509Cert_Body_FIXME_RDN_Malformed, "Distinguished name contains undecodable RDN element \"%s\"." % (OIDDB.RDNTypes.get(rdn_item.oid, str(rdn_item.oid))), commonness = Commonness.HIGHLY_UNUSUAL)
		else:
	...
	def _analyze_rdn(self, rdn):
		...
		if rdn.component_cnt > 1:
			judgements += SecurityJudgement(JudgementCode.X509Cert_Body_FIXME_RDN_MultiValuedRDN, "Distinguished name contains a multivalue RDN: %s" % (rdn.pretty_str), commonness = Commonness.HIGHLY_UNUSUAL, compatibility = Compatibility.LIMITED_SUPPORT)

		rdn_data = [ (rdn_item.oid, rdn_item.derdata) for rdn_item in rdn ]
		rdn_data_set = set(rdn_data)
		if len(rdn_data) != len(rdn_data_set):
			judgements += SecurityJudgement(JudgementCode.X509Cert_Body_FIXME_RDN_DuplicateSet, "Relative distinguished name contains identical value more than once in SET: %s" % (rdn.pretty_str), commonness = Commonness.HIGHLY_UNUSUAL, compatibility = Compatibility.LIMITED_SUPPORT)
		...
		if len(rdn_oids) != len(rdn_oids_set):
			...
			judgements += SecurityJudgement(JudgementCode.X509Cert_Body_FIXME_TODOREMOVEMEDuplicateOIDInMultivaluedRDN, "Multivalued relative distinguished name contains same OID more than once in: %s" % (rdn.pretty_str), commonness = Commonness.HIGHLY_UNUSUAL, compatibility = Compatibility.STANDARDS_DEVIATION, standard = standard)
	...
	def analyze(self, dn):
		...
		if len(all_cns) == 0:
			judgements += SecurityJudgement(JudgementCode.X509Cert_Body_FIXME_NoCN, "Certificate does not have any common name (CN) set.", commonness = Commonness.HIGHLY_UNUSUAL)
		elif len(all_cns) > 1:
			judgements += SecurityJudgement(JudgementCode.X509Cert_Body_FIXME_MultipleCN, "Certificate does have more than one common name (CN) set; in particular, %d CN fields were encountered." % (len(all_cns)), commonness = Commonness.UNUSUAL)
		...
		for rdn in dn:
			...
			if oidkey in seen_oid_keys:
				...
				judgements += SecurityJudgement(JudgementCode.X509Cert_Body_FIXME_DuplicateRDNs, "Distinguished name contains RDN element at least twice: %s (at element %s)" % (oidkey_str, rdn.pretty_str), commonness = Commonness.UNUSUAL, compatibility = Compatibility.LIMITED_SUPPORT)
			else:
				seen_oid_keys.add(oidkey)

		if dn.rdn_count > self._LARGE_RDN_AMOUNT:
			judgements += SecurityJudgement(JudgementCode.X509Cert_Body_FIXME_UnusuallyManyRDNs, "Distinguished name contains an unusually high amount of RDNs (%d)." % (dn.rdn_count), commonness = Commonness.UNUSUAL)
